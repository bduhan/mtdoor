...
class Zork(BaseCommand):
    ...
    def invoke(self, msg: str, node: str) -> str:
        ...
        match self.room[node]:
            ...
            case 4:
                if (
                    re.fullmatch(r"^(go|walk|run)\s(.*\s)*(sw|southwest)$", inp)
                    ...
                    re.fullmatch(r"^(go|walk|run)\s(.*\s)*(s|south)$", inp)
                    or re.fullmatch(r"^(s|south)$", inp)
                    ):
                    response += "The forest is too thick to go south.\n\n"
                elif (
                    re.fullmatch(r"^(go|walk|run)\s(.*\s)*(e|east)$", inp)
                    or re.fullmatch(r"^(e|east)$", inp)
                    ):
                    response += "The door is boarded and you cannot remove the boards.\n\n"
                elif (
                    re.fullmatch(r"^(go|walk|run)\s(.*\s)*(w|west)$", inp)
                ...
                elif re.fullmatch(r"(open|look|break|smash|go|enter)\s(.*\s)*door", inp):
                    response += "The door cannot be opened.\n\n"
                elif re.fullmatch(r"(remove|look|break|smash|take|get)\s(.*\s)*boards", inp):
                    ...
                    response += "It is clear that the owners must have been extremely wealthy. "
                    ...
                    self.room[node] = 1
                elif re.fullmatch(r"(read|look|open)\s(.*\s)*(mail|letter|leaflet)", inp):
            ...
            case 1:
                if (
                    ...
                    self.room[node] = 4
                elif re.fullmatch(r"(go|walk|run|exit|climb)\s(.*\s)*window", inp) or \
                    re.fullmatch(r"(leave|back|out).*", inp):
                    response += "You exit the room through the same window where you came in.\n\n"
                    self.room[node] = 4
                elif re.fullmatch(r"(turn|power|talk)\s(.*\s)*radios?", inp) or \
                    re.fullmatch(r"(turn|power|talk)\s(.*\s)*equipment", inp):
            ...
            case 8:
                if (
                    re.fullmatch(r"^(go|walk|run)\s(.*\s)*(ne|northeast)$", inp)
            ...
            case 9:
                if (
                    ...
                    re.fullmatch(r"^(go|walk|run)\s(.*\s)*(w|west|back)$", inp)
                    or re.fullmatch(r"^(w|west|back)$", inp)
                    ):
                    self.room[node] = 8
                elif (
                    re.fullmatch(r"^(go|walk|run)\s(.*\s)*(d|down)$", inp)
                    or re.fullmatch(r"^(d|down)$", inp)
                    ):
                    self.room[node] = 10
                elif re.fullmatch(r"(go|walk|run|down|enter|descend)\s(.*\s)*grating", inp):
                    self.room[node] = 10
                elif inp == "help":
                    return help()
                elif inp == "look":
            ...
            case 10:
                if (
                    ...
                    re.fullmatch(r"^(go|walk|run)\s(.*\s)*(u|up|back)$", inp)
                    ...
                    self.room[node] = 11
                elif re.fullmatch(r"(go|walk|run|down|enter|descend)\s(.*\s)*staircase", inp):
                    self.room[node] = 11
                ...
                elif re.fullmatch(r"(go|walk|down|scale|descend)\s(.*\s)*(stair|stairs|staircase)", inp):
                    self.room[node] = 11
        ...
        if self.room[node] == 1:
            ...
            response += "A small device with an antenna and a //\\ logo is lying on the desk."
        ...
        if self.room[node] == 10:
            response += "You are in a tiny cave with a dark, forbidding staircase leading down."
        ...
        if self.room[node] == 11:
            ...
            response += "Lying half buried in the mud is an old chest, bulging with jewels."
