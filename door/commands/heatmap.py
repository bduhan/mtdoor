...
class Heatmap(BaseCommand):
    ...
    def render_map(self):
        ...
        for node in self.interface.nodes.values():
            ...
            if node_data['id'] == myNodeId and not node_data["latitude"] and not node_data["longitude"]:
        ...
        heat_data = [
            ...
            if (
                node["lastHeard"] != None
                and datetime.now() - datetime.fromtimestamp(node["lastHeard"]) < timedelta(seconds=max_age)
                and node["hopsAway"] == 0
                and node["latitude"]
                and node["longitude"]
